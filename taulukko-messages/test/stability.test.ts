...
describe('#Stability test-', function  () {
 ...
 it('publish into a inexistent server with timeout but the server up before timeout',async function()
  {
    ...
    const publisher = await Publisher.create({
       server:"taulukko://localhost:" + DEFAULT_PORT, timeout:8000,topics:["echo"]
    }); 
    await publisher.open(); 
    ...
    const onNewMessage = async (message:Message)=>{
      try{
          ...
          assert.equal(message.topic,"echo","Topic need be the same topic in the publisher.send");
          assert.equal(message.data, "Hello World","Message need be Hello World");
  ...
  it.skip('Publisher with a persistent configuration',async  () => {

    //if there is "persistent" in the configuration, he needs to receive everything he lost since the last downtime
...
describe('#stability long time test-', function ()  {
  ...
  it('reconecting with a publisher with problems',async  () => {
    const NUMBER_OF_TESTS = 2;
    
    let times = 0;

    const server:Server = await initServer();

    assert.equal(server.publishers.length,0);

    let publisher:Publisher = await Publisher.create({ 
      server:"taulukko://localhost:" + DEFAULT_PORT,
      topics:["topic.helloWorld"],
      defaultLogLevel:LogLevel.ERROR
    });
    ...
    assert.equal(server.subscribers.length,0,"Must be 0 before subscriber.open");
    let subscriber:Subscriber = await Subscriber.create({ 
      server:"taulukko://localhost:" + DEFAULT_PORT,
      topics:["topic.helloWorld","unexistentTopic" ], defaultLogLevel:LogLevel.ERROR
    });
    assert.equal(server.subscribers.length,0,"Must be 0 before subscriber.open");
    ...
    assert.equal(server.publishers.length,0,"Need keep 0 after publisher close");
    assert.equal(server.subscribers.length,1,"Must keep subscriber");
    assert.equal( publisher.data.status,serviceStatus.STOPED,"Must be STOPED after publisher.close");
    ...
    await publisher.send("Hello World");  
    assert.ifError( await receiveTheMessage()); 
    assert.equal(server.publishers.length,0,"Must be zero after the subscriber.close");
    assert.equal(publisher.data.status,serviceStatus.STOPED,"Must be STOPED after  publisher.close");
